...
void Lab9::Update(float deltaTimeSeconds)
{
    ...
    // Why does this happen? How can you solve it?
    {
        ...
        RenderSimpleMesh(meshes["sphere"], shaders["LabShader"], modelMatrix, mapTextures["earth"], mapTextures["random"], true);
    }
...
void Lab9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix,
    Texture2D* texture1, Texture2D* texture2, bool isGlobe)
{
    ...
    {
        int loc_bool = glGetUniformLocation(shader->program, "exists_1");
        glUniform1i(loc_bool, exists_1);
    }
    ...
    {
        int loc_bool = glGetUniformLocation(shader->program, "exists_2");
        glUniform1i(loc_bool, exists_2);
    }

    {
        int loc_bool = glGetUniformLocation(shader->program, "is_globe");
        glUniform1i(loc_bool, isGlobe);
    }
    if (isGlobe) {
        float scale = .1f;
        int loc_int = glGetUniformLocation(shader->program, "time");
        glUniform1f(loc_int, scale * Engine::GetElapsedTime());
    }
